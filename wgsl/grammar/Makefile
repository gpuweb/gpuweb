...
EXTRAS := $(filter-out $(PARSER),$(wildcard $(SRC_DIR)/*.cpp))
OBJS := $(patsubst %.c,%.o,$(PARSER) $(EXTRAS))

# flags
ARFLAGS ?= rcs
override CFLAGS += -I$(SRC_DIR) -std=c17 -fPIC
override CXXFLAGS += -I$(SRC_DIR) -std=c++17 -fPIC
...
lib$(LANGUAGE_NAME).$(SOEXT): $(OBJS)
	$(CXX) -I$(SRC_DIR) -std=c++17 -fPIC $(LDFLAGS) $(LINKSHARED) $^ $(LDLIBS) -o $@
ifneq ($(STRIP),)
	$(STRIP) $@
endif
